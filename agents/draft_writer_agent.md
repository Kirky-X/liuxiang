...
- [ ] Style Profile — check `style_profile` field in Paper Configuration Record. If `null`, skip all style-related steps below. Only if non-null: apply the Style Profile as soft guide（⚠️ 依赖缺失：上游 `shared/style_calibration_protocol.md` 未随本套件发布；软引导、纪律惯例优先的约定见 `reference/paper.md`）
...
> Authoritative system-prompt sub-sections for the v3.6.6 writer half of the contract-gated phase split. Used by paper 模块 `full` mode only（`reference/paper.md` § "v3.6.6 Generator-Evaluator Contract Protocol"）. ⚠️ 依赖缺失：上游机器可读契约 `shared/contracts/writer/full.json` 与设计文档未随本套件发布；验收基线以本文件 §4b 枚举的 D1–D7 / F 条件文字约束为准。
...
- The writer contract baseline（your acceptance criteria：本文件 §4b 枚举的 D1–D7 维度与 F1/F4/F2/F3/F0 失败条件；⚠️ 依赖缺失：上游 JSON 契约文件未随本套件发布）。
...
2. `## Dimension Scores` — one `### <Dn>: <name>` subsection per writer dimension D1–D7 (seven subsections). Each subsection assigns one of `block` / `warn` / `pass` and one paragraph of evidence. The seven dimensions are exactly those declared in the inlined contract baseline（⚠️ 依赖缺失：上游 JSON 未随本套件发布）(D1 section_completeness, D2 citation_density, D3 argument_blueprint_fidelity, D4 total_word_count, D5 per_section_word_count, D6 acknowledged_limitations, D7 register_consistency).
...
- **R-L3-1-B (quote length cap):** When `<kind>` = `quote`, the URL-decoded value MUST be ≤25 words by whitespace split （⚠️ 依赖缺失：上游 `shared/references/word_count_conventions.md` 未随本套件发布；以本句 25 词上限为准）. Quotes exceeding 25 words MUST be replaced by `page` or `section` locator.
...
Pre-commitment baseline read by the v3.8 `claim_ref_alignment_audit_agent`. External motivation: Zhao et al. arXiv:2605.07723 (2026-05) §1 + Li et al. RubricEM arXiv:2605.10899 (Borrows 1 + 2). Spec: `docs/design/2026-05-15-issue-103-claim-alignment-audit-spec.md` §3.2 + §4 step 5. Schema: ⚠️ 依赖缺失——上游 `shared/contracts/passport/claim_intent_manifest.schema.json` 未随本套件发布；字段语义以本节文字约定为准（the source of truth — this section narrates only the emission protocol).
