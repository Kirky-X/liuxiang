...
search 模块的产物（Markdown 全文列表）是 paper 模块 Phase 1 `literature_strategist_agent` 与 pipeline 模块 Stage 1 的输入语料。最小交接格式：一个文献清单文件（Markdown），每篇一个条目，固定字段为**标题 / 来源（venue+标识符）/ 本地路径**：
...
| # | 标题 | 来源 | 标识符 | 本地路径 |
|---|------|------|--------|----------|
| 1 | Attention Is All You Need | arXiv | 2306.12345（arXiv ID / DOI / S2 ID 任一） | ./papers/attention-is-all-you-need.md |
| 2 | … | OpenAlex | DOI:10.xxxx/… | ./papers/….md |
```

- 已下载全文的条目必须给 `本地路径`（指向 `download_paper.py` 产出的 .md）；只拿到摘要的条目本地路径留空，并在来源列注明「仅摘要」。
- 该清单直接作为 `literature_strategist_agent`（见 `../agents/literature_strategist_agent.md`）的输入语料，或作为 pipeline Stage 1 → Stage 2 的交接产物（见 [`pipeline.md`](pipeline.md) Stage 1 输出约定）。
